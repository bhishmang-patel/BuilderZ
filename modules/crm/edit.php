...
if (!$id) {
    ...
    redirect('modules/crm/index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $crm->updateLead($id, [
            ...
            'source'    => $_POST['source'],
            // Updates to notes can be appended or overwritten, but original add.php puts notes in leads table
            'notes'     => $_POST['notes']
        ]);
...
<div class="pw">
    ...
    <div class="page-header">
        <div>
            ...
            <h1>Edit <em>Lead</em></h1>
        </div>
        <a href="view.php?id=<?= $id ?>" class="back-link">
            ...
            <div class="card-body">
                ...
                <div class="mf-row">
                    <div class="mf">
                        ...
                        <input type="text" name="full_name" required placeholder="e.g. Ramesh Kumar"
                               value="<?= htmlspecialchars($_POST['full_name'] ?? $lead['full_name']) ?>">
                    </div>
                    <div class="mf">
                        ...
                        <input type="tel" name="mobile" required placeholder="e.g. 9876543210" pattern="[0-9]{10}" maxlength="10" minlength="10" title="Please enter exactly 10 digits"
                               value="<?= htmlspecialchars($_POST['mobile'] ?? $lead['mobile']) ?>">
                    </div>
                </div>
                ...
                <div class="mf">
                    ...
                    <input type="email" name="email" placeholder="e.g. ramesh@email.com"
                           value="<?= htmlspecialchars($_POST['email'] ?? $lead['email']) ?>">
                </div>

                <div class="mf">
                    <label>Address</label>
                    <input type="text" name="address" placeholder="e.g. 123, Main Street, City"
                           value="<?= htmlspecialchars($_POST['address'] ?? $lead['address']) ?>">
                </div>
                ...
                <div class="source-grid">
                    ...
                    $sel_source = $_POST['source'] ?? $lead['source'] ?? 'Walk-in';
            ...
            <div class="card-body">
                <div class="mf">
                    ...
                    <textarea name="notes" rows="3"
                              placeholder="e.g. Interested in 3BHK on upper floors…"><?= htmlspecialchars($_POST['notes'] ?? $lead['notes'] ?? '') ?></textarea>
                </div>
            </div>
            <div class="form-foot">
                <a href="view.php?id=<?= $id ?>" class="btn-cancel">
