...
//    protected $rIndex;
    ...
    public function __construct()
    {
        ...
        $this->XF = [  // styles 0 - num fmt, 1 - align, 2 - font, 3 - fill, 4 - font color, 5 - bgcolor, 6 - border, 7 - font size
            [self::N_NORMAL, self::A_DEFAULT, self::F_NORMAL, self::FL_NONE, 0, 0, '', 0, 0],
            [self::N_NORMAL, self::A_DEFAULT, self::F_NORMAL, self::FL_GRAY_125, 0, 0, '', 0, 0], // hack
        ];
...
// Anchor: LeftColumn, LeftOffset, TopRow, TopOffset, RightColumn, RightOffset, BottomRow, BottomOffset
                ...
                foreach ($this->XF as $xf) {
                    ...
                    // fonts
                    $F_KEY = $xf[2] . '-' . $xf[4] . '-' . $xf[7] . '-' . $xf[8];
                    if (isset($F_KEYS[$F_KEY])) {
                        $F_ID = $F_KEYS[$F_KEY];
                    } else {
                        $F_ID = $F_KEYS[$F_KEY] = count($FONTS);
                        $fontName = $xf[8] ? $xf[8] : $this->defaultFont;
                        $FONTS[] = '<font><name val="' . $fontName . '"/><family val="2"/>'
                ...
                foreach ($r as $v) {
                        ...
                        if (strpos($v, "\0") === 0) { // RAW value as string
                            ...
                            if (strpos($v, '<') !== false) { // tags?
                                ...
                                if (preg_match('/<style([^>]+)>/', $v, $m)) {
                                    ...
                                    if (preg_match('/ font-family="([^"]+)"/', $m[1], $m2)) {
                                        $FN = $m2[1];
                                    }
                                }
                    ...
                    if (($N + $A + $F + $FL + $FS > 0) || $BR !== '') {
                        ...
                        if ($cs === 0) {
                            ...
                            $this->XF[] = [$N, $A, $F, $FL, $C, $BG, $BR, $FS, $FN];
                        }
                    }
